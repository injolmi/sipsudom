		...
		if(in_array($ekstensi, $ekstensi_diperbolehkan) === true and in_array($ekstensi_ttd, $ekstensi_diperbolehkan_ttd) === true){
			...
			if($ukuran < 1044070 and $ukuran_ttd < 1044070){
				...
				$update = mysqli_query($koneksi, "UPDATE 
					pegawai set nama_pegawai='$nama_pegawai', jabatan='$jabatan', no_telepon='$no_telepon', email='$email' , username='$username', password='$password', foto_pegawai='$foto_pegawai', ttd_pegawai='$ttd_pegawai' WHERE npak='$npak'");
				if($update){
